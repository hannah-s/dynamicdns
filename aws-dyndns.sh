...
if [ $NEWIP != $CURRENTIP ]; then
	echo "The external IP address has changed to $NEWIP. Updating the A record for $MYHOSTNAME."
	sed "s/MYHOSTNAME/$MYHOSTNAME/g; s/CHANGETIME/$CHANGETIME/g; s/NEWIP/$NEWIP/g" ~/dynamicdns/template.json > ~/dynamicdns/$MYHOSTNAME.json 
